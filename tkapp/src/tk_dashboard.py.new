...
# ---------------- Map canvas: kept as a labeled DEMO, no real GPS invented ----------------
class MapCanvas(tk.Canvas):
    def __init__(self, parent, **kw):
        super().__init__(parent, bg="#1a2030", highlightthickness=0, **kw)
        self._car_t = 0.0
        self._route = [(30, 85), (80, 65), (140, 55), (200, 45), (260, 35), (290, 28)]
    ...
    def _draw(self):
        ...
        for x in range(0, 301, 30):
            self.create_line(x * sx, 0, x * sx, h, fill="#2a3040", width=1)
        for y in range(0, 101, 25):
            self.create_line(0, y * sy, w, y * sy, fill="#2a3040", width=1)

        pts_flat = []
        for (rx, ry) in self._route:
            pts_flat += [rx * sx, ry * sy]
        self.create_line(*pts_flat, fill="#4a5568", width=int(7 * sy), smooth=True)
        self.create_line(*pts_flat, fill="#718096", width=int(2 * sy), dash=(8, 4), smooth=True)

        cx, cy = self._interp(self._car_t)
        cx, cy = cx * sx, cy * sy
        self.create_oval(cx - 8, cy - 8, cx + 8, cy + 8, outline=BLUE, width=1)
        self.create_oval(cx - 5, cy - 5, cx + 5, cy + 5, fill=BLUE, outline="")

        # Clear "this is a demo" label - not real GPS
        self.create_text(w - 6, h - 4, text="DEMO - GPS not connected", fill="#f59e0b",
                          font=("Courier", 8, "bold"), anchor="se")

    def _animate(self):
        self._car_t = (self._car_t + 0.001) % 1.0
        self._draw()
        self._after_job = self.after(60, self._animate)
...
class DrowsiGuardApp(tk.Tk):
    ...
    def _build_left(self, parent):
        left = tk.Frame(parent, bg=PANEL, width=190)
    ...
    def _build_center(self, parent):
        ...
        self.video_canvas = VideoCanvas(center, height=260)
        self.video_canvas.pack(fill="both", expand=True, padx=4)

        thresh_frame = tk.Frame(center, bg=BG)
        thresh_frame.pack(anchor="w", padx=4, pady=6)
    ...
    def _build_right(self, parent):
        tk.Frame(parent, bg=BORDER, width=1).pack(side="left", fill="y")
        right = tk.Frame(parent, bg=PANEL, width=220)
        right.pack(side="left", fill="y")
        right.pack_propagate(False)

        tk.Label(right, text="Alert:", bg=PANEL, fg=MUTED, font=("Courier", 7, "bold")).pack(anchor="w", padx=8, pady=(6, 4))

        self.alerts_container = tk.Frame(right, bg=PANEL)
        self.alerts_container.pack(fill="x")

        self.no_alert_label = tk.Label(self.alerts_container, text="No active alerts", bg=PANEL, fg=GREEN, font=("Courier", 8, "bold"))
        self.no_alert_label.pack(anchor="w", padx=8, pady=6)

        rec = tk.Frame(right, bg=CARD, highlightbackground=BORDER, highlightthickness=1)
    ...
    def _build_bottom(self):
        tk.Frame(self, bg=BORDER, height=1).pack(fill="x")
        bottom = tk.Frame(self, bg=BG, height=130)
        bottom.pack(fill="x")
        bottom.pack_propagate(False)

        veh = tk.Frame(bottom, bg=PANEL, width=190)
        veh.pack(side="left", fill="y")
        veh.pack_propagate(False)
        tk.Label(veh, text="Vehicle Status (DEMO)", bg=PANEL, fg="#f59e0b", font=("Courier", 7, "bold")).pack(anchor="w", padx=8, pady=(6, 0))
        speed_frame = tk.Frame(veh, bg=PANEL)
        speed_frame.pack(expand=True)
        tk.Label(speed_frame, text="--", bg=PANEL, fg=MUTED, font=("Courier", 36, "bold")).pack()
        tk.Label(speed_frame, text="OBD-II not connected", bg=PANEL, fg=MUTED, font=("Courier", 7)).pack()

        tk.Frame(bottom, bg=BORDER, width=1).pack(side="left", fill="y")

        map_frame = tk.Frame(bottom, bg=PANEL)
        map_frame.pack(side="left", fill="both", expand=True, padx=4, pady=4)
        top_row = tk.Frame(map_frame, bg=PANEL)
        top_row.pack(fill="x", padx=4, pady=(4, 0))
        tk.Label(top_row, text="Live Location (DEMO)", bg=PANEL, fg="#f59e0b", font=("Courier", 7, "bold")).pack(side="left")
        ...
        map_canvas = MapCanvas(map_frame)
        map_canvas.pack(fill="both", expand=True, padx=4, pady=(0, 4))
    ...
    def _build_footer(self):
        ...
        self._footer_dots = {}
        for key, label in [("system", "System OK"), ("gps", "GPS: Not Connected"), ("sensors", "Sensors Online")]:
        ...
        self._footer_dots["gps"][0].configure(fg=MUTED)  # honestly gray, not green
    ...
    def process_one_frame(self, frame):
        ...
        if results.multi_face_landmarks:
            ...
            self.draw_face_mesh(frame, face)
